...
// Role constants for RBAC. Hierarchy ranks: owner (100) > admin (80) > client / custom roles (30) > viewer (10)
const (
	RoleOwner  = "owner"
	RoleAdmin  = "admin"
	RoleClient = "client"
...
// Role rank constants define the strict hierarchy used by user management. A caller may only create/edit/delete users whose role rank is STRICTLY LOWER than their own.
const (
	RoleRankOwner  = 100
	RoleRankAdmin  = 80
	RoleRankClient = 30
...
const (
	...
	// Deprecated: kept for backwards compatibility with roles seeded before the audit-log split. Not enforced by any route; the audit log endpoint requires PermViewAuditLogs.
	...
	// Deprecated: kept for backwards compatibility with roles seeded before the granular split. Migrated to PermViewUsers/CreateUsers/EditUsers/ DeleteUsers.
	PermManageUsers = "can_manage_users"
)

// AllPermissions is the complete set of permission keys granted implicitly to the owner and admin accounts. Used for /me responses and middleware checks. NOTE: backups are NOT in this list — DB backup access is hardcoded to the owner role only (see router.go / DownloadBackupHandler).
...
// PermissionsGrantedBy maps a coarse/deprecated permission to the granular keys it implicitly grants. Used so roles seeded with can_manage_users keep working until they are migrated to the granular user permissions.
...
// UserPreferences holds the per-user personalization settings: accent color, theme mode, UI language, bot emoji rendering and glassmorphism blur.
...
type Node struct {
	...
	// AvailableServers groups cached server names by their provider tag (friendly provider name or the subscription URL host). Keyed by provider so the web UI and bot can render grouped server lists directly; legacy flat arrays are normalized to a single provider-less group on read.
	AvailableServers map[string][]string `json:"available_servers" db:"available_servers"`
	// ServerProviders maps a cached server name to its provider name (friendly name from subscription_providers, or the subscription URL host as fallback). Reported by the agent so UIs can group servers by provider.
...
// SubscriptionProvider maps a subscription URL host (domain) to the friendly provider name shown across the web UI, bot and CLI when grouping servers.
